...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if (!empty($_COOKIE['save'])) {
    ...
    if (!empty($_COOKIE['password'])) {
        $messages[] = sprintf('Вы можете <a href="login7.php">войти</a> с логином <strong>%s</strong>
...
// предварительно санитизовав.
  ...
  $uid = $_SESSION['uid'];
  $res= $db->query("SELECT fio, email, year, gender, biography FROM application7 WHERE id = $uid");
  foreach($res as $el){
  ...
  $res= $db->query("SELECT num FROM Languages7 WHERE id = $uid");
  $sup = array();
  foreach($res as $el){
    $sup[]=(int)strip_tags($el['num']);
  }
  $sp = implode('',$sup);
  $values['power'] =$sp;
...
// Иначе, если запрос был методом POST, т.е. нужно проверить данные и сохранить их в XML-файл.
  ...
  $errors = FALSE;
  if (empty($_POST['fio']) || !preg_match('/^\s*\w+[\w\s-]*$/', $_POST['fio'])) {
...
try {
  $uid = $_SESSION['uid'];
  $stmt = $db->prepare("UPDATE application7 SET fio = ?, email = ?, year = ?, gender = ?, biography = ? WHERE id = $uid");
  $stmt -> execute([$_POST['fio'],$_POST['email'],$_POST['year'],$_POST['gender'],$_POST['biography']]);
  
  $db->query("DELETE FROM Languages7 WHERE id = $uid");
  $stmt = $db->prepare("INSERT INTO Languages7 SET id = ?, num = ?");
...
try {
  ...
  $stmt = $db->prepare("INSERT INTO application7 SET fio = ?, email = ?, year = ?, gender = ?, biography = ?");
  $stmt -> execute([$_POST['fio'],$_POST['email'],$_POST['year'],$_POST['gender'],$_POST['biography']]);

  $id = $db->lastInsertId();
  $stmt = $db->prepare("INSERT INTO login7 SET id = ?, login = ?, password = ?");
