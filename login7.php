...
// Иначе, если запрос был методом POST, т.е. нужно сделать авторизацию с записью логина в сессию.
  ...
  $sel = $db->query("SELECT login FROM login7 WHERE login = $l");
    foreach($sel as $el)
      $login=$el['login'];
  $sel = $db->query("SELECT password FROM login7 WHERE login = $l");
    foreach($sel as $el)
      $pas=$el['password'];
  ...
  $p2=$db->quote($p);
  $sel = $db->query("SELECT id FROM login7 WHERE login=$l AND password=$p2");
  foreach($sel as $el)
    $id = (int)$el['id'];
